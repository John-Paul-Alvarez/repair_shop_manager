...
export default function CustomerTrackingPage() {
  ...
  useEffect(() => {
    let active = true;
    api("/tracking/" + token).then((data) => { if (active) { setRepair(data.repair); setShop(data.shop); } }).catch((loadError) => active && setError(loadError.message)).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [token]);
  ...
  return <main className="tracking-page page-container">
    ...
    {contactOpen && <div className="modal-backdrop"><section className="manager-modal" role="dialog" aria-modal="true" aria-labelledby="contact-title"><button className="modal-close" onClick={() => setContactOpen(false)} aria-label="Close">×</button><h2 id="contact-title">Contact {shop?.name ?? "the repair shop"}</h2><p>When you contact the shop, share this order number so staff can quickly find your repair.</p><div className="contact-order"><strong>{repair.number}</strong><button className="button button-outline" onClick={copyOrder}>Copy order number</button></div>{(shop?.publicPhone || shop?.publicEmail) ? <div className="contact-actions">{shop.publicPhone && <a className="button button-primary" href={"tel:" + shop.publicPhone}>Call the shop</a>}{shop.publicEmail && <a className="button button-outline" href={"mailto:" + shop.publicEmail + "?subject=" + encodeURIComponent("Repair " + repair.number)}>Email the shop</a>}</div> : <p className="field-hint">No public phone or email has been added yet. Please use the contact information you received when you dropped off your device.</p>}{copied && <p role="status" className="save-success">{copied}</p>}</section></div>}
  </main>;
}
