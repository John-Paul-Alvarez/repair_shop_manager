...
export default function WorkOrderDetailsPage() {
  ...
  async function createTrackingLink() {
    ...
    try {
      const result = await api("/work-orders/" + orderId + "/tracking-link", {});
      setTrackingLink(new URL(result.path, window.location.origin).href);
    } catch (trackingLoadError) { setTrackingError(trackingLoadError.message); } finally { setTrackingBusy(false); }
  }
  async function copyTrackingLink() {
    try { await navigator.clipboard.writeText(trackingLink); setSuccess("Customer tracking link copied."); }
    catch { setTrackingError("Select the link and copy it manually."); }
  }
  ...
  return <>
    ...
    <main className="details-main page-container">
      {loading ? <div className="details-state" role="status">Loading work order…</div> : error ? <div className="details-state"><h1>Work order unavailable</h1><p role="alert">{error}</p><button className="button button-dark" onClick={() => void load()}>Try again</button><Link to="/work-orders">Back to work orders</Link></div> : <>
        ...
        {!technician && <section className="details-card tracking-card"><div><p className="eyebrow">Customer access</p><h2>Share a repair tracking link</h2><p>Creates a private link that expires in 30 days. Creating another link replaces the previous one.</p></div>{trackingLink ? <div className="tracking-result"><label htmlFor="tracking-link">Customer tracking link</label><input id="tracking-link" readOnly value={trackingLink} onFocus={(event) => event.target.select()} /><button className="button button-primary" onClick={copyTrackingLink}>Copy link</button><button className="text-button" onClick={() => void createTrackingLink()} disabled={trackingBusy}>{trackingBusy ? "Replacing…" : "Create replacement link"}</button></div> : <button className="button button-outline" onClick={() => void createTrackingLink()} disabled={trackingBusy}>{trackingBusy ? "Creating…" : "Create tracking link"}</button>}{trackingError && <p className="form-error" role="alert">{trackingError}</p>}</section>}
